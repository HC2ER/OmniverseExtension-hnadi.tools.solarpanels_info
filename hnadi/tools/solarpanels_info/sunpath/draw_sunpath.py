...
from .draw_sphere import DrawSphere 
...
class DrawSunpath:
    ...
    def draw_sunpath(self):
        self.draw_anydate_path(self.pathmodel, self.pathmodel.datevalue, cl(255,255,0), 1.5)
        self.draw_paths()
        self.draw_compass()
        sun = DrawSphere()
        sun.scale = self.scale
        sun.pathmodel = self.pathmodel
        sun.draw_movable_sphere()
    ...
    def sort_points(self, points):
        ...
        if len(x_points) != 0:
            z_min = min(x_points, key=lambda p: p[2])
            id = points.index(z_min) + 1
    ...
    def generate_circle_pts(self, offset, step):
        ...
        for angle in range(0, 361, step):
            ...
            y = self.origin[2] + self.scale * math.sin(sep * angle) * offset
            points.append([round(x, 3),round(y, 3) ,0])
        return points
    ...
    def draw_drections(self):
        ...
        for i in range(4):
            ...
            with sc.Transform(transform=sc.Matrix44.get_translation_matrix(x, y, z)):
                sc.Label(text[i], alignment=ui.Alignment.CENTER, color=cl(255,255,0), size=20)
    ...
    def draw_drection_mark(self):
        ...
        for p1, p2 in zip(points_a, points_b):
            sc.Curve(
                [p1, p2], thicknesses=[1.5], colors=[cl(255,255,0)], curve_type=sc.Curve.CurveType.LINEAR
            )
        ...
        for i in range(12):
            x, y, z = points_t[i]
            with sc.Transform(transform=sc.Matrix44.get_translation_matrix(x, y, z)):
                sc.Label(text[i], alignment=ui.Alignment.CENTER, color=cl(255,255,0), size=12)
    ...
    def show_info(self):
        ...
        """
        anchor = self.generate_circle_pts(1.45, 90)
        anchor_e = anchor[0]
        anchor_w = anchor[2]
        anchor_n = anchor[3]
